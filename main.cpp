...
// Class to represenat an image as a 3D matrix
class Image {
private:
...
Image applyBlur(const Image& input) {
    ...
    int width = input.getWidth();
    int channels = input.getChannels();
    Image output(width, height, channels); // border pixels stay 0 by default
    ...
    for (int y = 1; y <= height - 2; y++) {
        for (int x = 1; x <= width - 2; x++) {
            for (int c = 0; c < channels; c++) {
                ...
                output(y, x, c) = sum / 9; // integer division intended
            }
        }
    }
...
Image rotate90(const Image& input) {
    ...
    int width = input.getWidth();
    int channels = input.getChannels();
    Image output(height, width, channels); // العرض والطول متبدلين
